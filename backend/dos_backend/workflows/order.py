...
def apply_order_constraints(
    ...
    # If max_stock provides more coverage than shelf life allows, any large proposal
    # will be blocked (or reduced) by the shelf life penalty below.  Surfacing this
    # mismatch early helps the user reconfigure max_stock to a sustainable level.
    if shelf_life_days > 0 and daily_sales_avg > 0:
        max_stock_days_coverage = max_stock / daily_sales_avg
        # Allow 10 % tolerance to avoid spurious warnings
...
def calculate_daily_sales_average(
    ...
    if transactions:
        ...
        # Group by date for efficient day-by-day replay
        from itertools import groupby
        _by_day: dict = {
            day: list(grp)
            for day, grp in groupby(_window_txns, key=lambda t: t.date)
        }

        for day in calendar_days:
            # Check OOS at the START of the day (before applying that day's events)
            if day not in oos_override_days:
            ...
            # Apply events of this day to advance the running state
            for txn in _by_day.get(day, []):
                ev = txn.event
                if ev == EventType.SNAPSHOT:
                    oh = txn.qty
                    oo = 0
                elif ev == EventType.ORDER:
                    oo += txn.qty
                elif ev == EventType.RECEIPT:
                    oo = max(0, oo - txn.qty)
                    oh += txn.qty
                elif ev == EventType.SALE:
                    oh = max(0, oh - txn.qty)
                elif ev == EventType.WASTE:
                    oh = max(0, oh - txn.qty)
                elif ev == EventType.ADJUST:
                    oh = max(0, txn.qty)
            oh = max(0, oh)
            oo = max(0, oo)
