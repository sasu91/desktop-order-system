...
def normalize_ean_13(ean: str) -> str:
    """Convert a 12-digit EAN to its canonical 13-digit form (append check digit).

    ML Kit (and all physical scanners) always return the full EAN-13 string
    including the check digit.  When a SKU is stored with a 12-digit EAN in the
    CSV/DB, the barcode generated by python-barcode encodes 13 digits (the 12
    content digits + the computed check digit).  Scanning that barcode returns
    the 13-digit value, so we must normalise 12-digit stored EANs to 13 digits
    before writing them to the Android Room cache or comparing them for lookup.

    Args:
        ean: A 12- or 13-digit EAN string (already validated, all digits).
             Any other length is returned unchanged.

    Returns:
        13-digit EAN string, or the original value if length is not 12 or 13.
    """
    ...
    if len(ean) != 12:
        return ean
    # EAN-13 check digit: alternating weights 1,3 on positions 0-11 (0-indexed)
    total = sum(int(d) * (3 if i % 2 != 0 else 1) for i, d in enumerate(ean))
