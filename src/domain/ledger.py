...
class StockCalculator:
    ...
    @staticmethod
    def calculate_stock_series(
        sku: str,
        dates: List[date],
        transactions: List[Transaction],
        sales_records: Optional[List[SalesRecord]] = None,
        sku_transactions: Optional[List[Transaction]] = None,
        sku_sales: Optional[List[SalesRecord]] = None,
    ) -> Dict[date, "Stock"]:
        """
        Compute stock state for multiple dates in a single incremental pass.

        Complexity: O(T log T + D)  vs  O(D × T log T) for D repeated calculate_asof calls.
        Semantics are identical to calculate_asof: events with date < target_date are applied.

        Args:
            sku: SKU identifier.
            dates: Dates to compute stock for (any order; internally sorted).
            transactions: Full transaction list (or pass sku_transactions instead).
            sales_records: Full sales list (or pass sku_sales instead).
            sku_transactions: Pre-filtered transactions for this SKU — avoids rescanning.
            sku_sales: Pre-filtered sales for this SKU — avoids rescanning.

        Returns:
            Dict mapping each requested date → Stock object.
        """
        if not dates:
            return {}

        # Use pre-filtered data when caller already has it (avoids O(T) scan each call)
        if sku_transactions is not None:
            base_txns = list(sku_transactions)
        else:
            base_txns = [t for t in transactions if t.sku == sku]

        if sku_sales is not None:
            _sku_sales = sku_sales
        elif sales_records:
            _sku_sales = [s for s in sales_records if s.sku == sku]
        else:
            _sku_sales = []

        # Merge sales as synthetic SALE transactions (same as calculate_asof)
        sale_events = [
            Transaction(date=s.date, sku=s.sku, event=EventType.SALE, qty=s.qty_sold)
            for s in _sku_sales
        ]
        base_txns.extend(sale_events)
        base_txns = StockCalculator._sort_transactions(base_txns)

        # Incremental walk: advance pointer through sorted transactions, snapshot at each target date
        on_hand = 0
        on_order = 0
        unfulfilled_qty = 0
        txn_idx = 0
        n_txns = len(base_txns)

        result: Dict[date, Stock] = {}
        for target_date in sorted(set(dates)):
            # Apply all events strictly before target_date
            while txn_idx < n_txns and base_txns[txn_idx].date < target_date:
                txn = base_txns[txn_idx]
                if txn.event == EventType.SNAPSHOT:
                    on_hand = txn.qty
                    on_order = 0
                elif txn.event == EventType.ORDER:
